...
def main():
    render_page_header(
        "🔮 Sourcerer Control Panel", "Manage your AI-powered RAG backend."
    )
    ...
    if is_healthy:
        st.markdown(
            '<span class="status-badge status-ok">Backend Online</span>',
            unsafe_allow_html=True,
        )
        st.success("Successfully connected to the Sourcerer backend API.")
    else:
        st.markdown(
            '<span class="status-badge status-error">Backend Offline</span>',
            unsafe_allow_html=True,
        )
        st.error(
            "Failed to connect to the backend. Please ensure the backend server is running on port 8000."
        )
