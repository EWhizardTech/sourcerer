...
from sourcerer_core.rerank import rerank
...
class VectorStoreService:
    ...
    def search(
        ...
        # Overfetch candidates so the cross-encoder has room to reorder.
        fetch_k = max(k, k * settings.RERANK_OVERFETCH) if settings.RERANK_ENABLED else k
        ...
        try:
            ...
            hits = [
                {"score": hit.score, "point_id": str(hit.id), **(hit.payload or {})}
                for hit in results.points
            ]
            return rerank(query_text, hits, top_k=k)
