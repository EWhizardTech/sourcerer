...
from sourcerer_core.embedding.embedding_service import EmbeddingService
...
def test_full_pipeline(file_path: str):
    ...
    print(f"\n--- [2] CHUNKING ---")
    mock_metadata = {"course_code": "TEST101", "year": "2024", "source": "document"}
    ...
    for i, chunk in enumerate(embedded_chunks[:2]):
        ...
        print(
            f"  Vector (first 5): {short_vector} ... (total: {len(chunk['dense_vector'])})"
        )
