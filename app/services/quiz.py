...
_GROQ_CLIENTS = [
    Groq(api_key=key.strip())
    for key in settings.GROQ_API_KEY.split(",")
    if key.strip()
]
...
def _get_next_groq_client() -> Groq | None:
    """Return a Groq client using round-robin selection."""
    global _GROQ_CLIENT_INDEX
    if not _GROQ_CLIENTS:
        return None
    with _GROQ_CLIENT_LOCK:
        client = _GROQ_CLIENTS[_GROQ_CLIENT_INDEX]
        _GROQ_CLIENT_INDEX = (_GROQ_CLIENT_INDEX + 1) % len(_GROQ_CLIENTS)
        return client


def _is_rate_limit_error(exc: Exception) -> bool:
    """Detect 429/rate-limit errors from provider responses."""
    err_text = str(exc).lower()
    return "rate_limit" in err_text or "429" in err_text


def _build_context_for_llm(chunks: list[dict[str, Any]], max_chars: int = 12000) -> str:
    """Build bounded retrieval context for LLM quiz generation."""
    parts: list[str] = []
    current_len = 0
    for chunk in chunks:
        ...
        if not text:
            continue
        block = f"[chunk_id={chunk_id}] [source={source}]\n{text[:1200]}\n"
...
def _normalize_llm_mcq_item(
    item: dict[str, Any],
    valid_chunk_ids: set[str],
) -> dict[str, Any] | None:
    """Normalize and validate one LLM-generated MCQ item."""
    ...
    return {
        ...
        "source_chunk_ids": normalized_source_ids,
    }


def _build_mcqs_with_groq(
    chunks: list[dict[str, Any]],
    num_questions: int,
    query: str,
) -> list[dict[str, Any]]:
    """Generate MCQs via Groq for higher semantic quality."""
    if not _GROQ_CLIENTS or not chunks or num_questions <= 0:
        return []

    context = _build_context_for_llm(chunks)
    if not context:
        return []

    valid_chunk_ids = {str(chunk.get("chunk_id", "")) for chunk in chunks}

    system_prompt = (
        "You are an expert educator creating practice MCQs for learning. "
        "Generate clear, accurate, and meaningful questions from the provided context."
    )
    user_prompt = (
        f"User query: {query}\n"
        f"Generate up to {num_questions} MCQs.\n"
        "Rules:\n"
        "1) Use only facts from context.\n"
        "2) Each item must have 1 correct answer and 3 plausible distractors.\n"
        "3) Avoid trivial wording and duplicates.\n"
        "4) Keep options concise and non-overlapping.\n"
        "5) source_chunk_ids must reference chunk_id values from context.\n\n"
        "If context is limited, return fewer high-quality items instead of guessing.\n\n"
        "Return strict JSON with this shape:\n"
        "{\"items\": [{\"question\": str, \"answer\": str, \"options\": [str, str, str, str], "
        "\"difficulty\": \"Easy|Medium|Hard\", \"source_chunk_ids\": [str]}]}\n\n"
        f"Context:\n{context}"
    )

    fallback_prompt = (
        f"User query: {query}\n"
        f"Generate up to {num_questions} practice MCQs from the context.\n"
        "Output only valid JSON (no markdown) with key 'items'.\n"
        "Each item must include: question, answer, options (4), difficulty, source_chunk_ids.\n"
        "Do not include explanations.\n\n"
        f"Context:\n{context}"
    )

    def _try_parse_items(raw_content: str) -> list[dict[str, Any]]:
        """Parse raw model output into MCQ items with light JSON extraction."""
        ...
        try:
            ...
            if start != -1 and end != -1 and end > start:
                try:
                    parsed_obj = json.loads(content[start : end + 1])
                except Exception:
                    return []
            else:
                return []
    ...
    attempts = max(len(_GROQ_CLIENTS), 1)
    for attempt in range(attempts):
        ...
        try:
            ...
            raw_items = _try_parse_items(response.choices[0].message.content or "")
            ...
            for raw_item in raw_items:
                if not isinstance(raw_item, dict):
                    continue
                normalized = _normalize_llm_mcq_item(raw_item, valid_chunk_ids)
                if normalized is not None:
                    normalized_items.append(normalized)
                if len(normalized_items) >= num_questions:
                    break

            if normalized_items:
                return normalized_items[:num_questions]
            continue

        except Exception as exc:
            if _is_rate_limit_error(exc) and attempt < attempts - 1:
                logger.warning("Groq rate-limited during quiz generation. Retrying another key.")
                time.sleep(1)
                continue
            logger.warning("Groq quiz generation failed: %s", exc)
            return []
...
def build_mcqs(
    ...
    # Groq-first path for semantically stronger questions and distractors.
    groq_mcqs = _build_mcqs_with_groq(
        chunks=chunks,
        num_questions=num_questions,
        query=query,
    )
    if len(groq_mcqs) >= num_questions:
        logger.info("Generated %d MCQs via Groq LLM", len(groq_mcqs))
        return groq_mcqs[:num_questions]

    if groq_mcqs:
        logger.info(
            "Generated %d MCQs via Groq LLM; topping up with NLP fallback",
            len(groq_mcqs),
        )
    else:
        logger.warning("Groq MCQ generation unavailable, falling back to NLP pipeline")
    ...
    # Merge Groq-first items with NLP items, preserving quality and target count.
    merged_mcqs = deduplicate_mcqs(groq_mcqs + mcqs, threshold=0.7)
    final_mcqs = merged_mcqs[:num_questions]
    ...
    logger.info(
        ...
        len(chunks),
        attempted,
        len(merged_mcqs) - len(final_mcqs),
    )
