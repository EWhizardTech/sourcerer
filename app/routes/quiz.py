...
from app.services.quiz import build_mcqs
...
async def generate_quiz(request: QuizGenerateRequest) -> list[QuizItem]:
    ...
    try:
        ...
        # Reuse the exact retrieval endpoint flow (LangGraph) to gather quiz context.
        retrieve_count = max(request.num_questions * 2, 10)
        filter_payload = request.filters.model_dump(exclude_none=True)
        if filter_payload:
            logger.warning(
                "Quiz filters are not directly applied in graph retrieval flow; query=%s filters=%s",
                request.query,
                filter_payload,
            )
        ...
        chunks = retrieval_result.get("structured_chunks", [])
        
        if not chunks:
            if filter_payload and not request.allow_unfiltered_fallback:
                raise HTTPException(
                    status_code=404,
                    detail=(
                        "No relevant content found via retrieval graph for this query, "
                        "and allow_unfiltered_fallback is disabled."
                    ),
                )
            ...
            if not chunks:
                ...
                raise HTTPException(
                    status_code=404,
                    detail="No relevant content found in retrieval graph output for this query"
                )
...
async def health_check() -> dict[str, str]:
    ...
    try:
        ...
        from app.services.quiz import _ensure_models_loaded
