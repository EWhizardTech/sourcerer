...
from sourcerer_core.retrieval_service import retrieval_service
...
async def generate_quiz(request: QuizGenerateRequest) -> list[QuizItem]:
    ...
    try:
        ...
        # Retrieve relevant chunks directly from the vector store (filters apply).
        ...
        if not chunks and filter_payload and request.allow_unfiltered_fallback:
            logger.info(
                "No filtered chunks for query=%s; retrying without filters.",
                request.query,
            )
            chunks = retrieval_service.retrieve_chunks(
                query=request.query,
